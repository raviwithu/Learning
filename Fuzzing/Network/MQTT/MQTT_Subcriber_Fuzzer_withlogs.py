...
def pre_test_callback(target, fuzz_data_logger, session, sock, *args, **kwargs):
    #target.send("\x10\x23\x00\x04\x4d\x51\x54\x54\x04\x02\x00\x3c\x00\x17\x6d\x6f\x73\x71\x2d\x79\x56\x77\x50\x54\x67\x37\x76\x70\x7a\x62\x4f\x36\x6f\x36\x56\x71\x58")
...
s_block_end()
    ...
    #session.connect(s_get("MQTT_PUB_MESSAGE"))
...
session = Session(target=Target(connection=TCPSocketConnection("192.168.129.200",1883)),sleep_time=2,post_test_case_callbacks=[post_test_callback], fuzz_loggers=my_logger)
session.connect(s_get("MQTT_CONNECT_COMMAND"))
session.fuzz()
