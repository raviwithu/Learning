...
'''
Script are modified from https://gist.github.com/N0dr4x/ffe99618a738978605719ce525a33042
...
Usage & example :
   ...
   >>> sess = TcpSession(('172.28.1.253',88))
   >>> sess.connect()
...
sess = TcpSession(('172.28.1.253',88))
sess.connect()
pkts=sniff(offline="MRISI_ServiceLaptop_Communication.pcapng", filter='dst host 172.28.1.253  and tcp port 88')
for pkt in pkts:
    if TCP in pkt and str(pkt[TCP].payload).find("SWIFT")>0:
        sess.send(b'0000'+b'\r\n\r\n')
        sess.send(bytes(pkt[TCP].payload) * 1000 + b'\r\n\r\n')
#    if count > Max_count:
#        break
    count = count + 1
sess.close()
